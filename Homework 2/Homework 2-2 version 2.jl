...
for i = 1:numPanels
    ...
    for j = 1:numPanels
        ...
        rij,rijPlusOne,Beta,theta_i,theta_j = getInformation(currentPanel,currentControlPoint,controlPointPanel,i,j)
        ...
        if i == 1 || i == numPanels
            ...
            ANplus1j = Beta * sin(theta_i - theta_j) - log(rijPlusOne/rij) * cos(theta_i - theta_j)

            # Bottom right corner of matrix A
            for z = 1:numPanels
                # Same for this array
                # BUT... we will need to make an additional sum over all of the z panels
                currentPanel_z = [panels[z,1:2],panels[z+1,1:2]] # Define the new panel we're using
                # Get the information (only used within this loop)
                riz,rizPlusOne,Beta_z,theta_i,theta_z = getInformation(currentPanel_z,currentControlPoint,controlPointPanel,i,z)

                # Inside the summation
                ANplus1Nplus1[z] = Beta_z * cos(theta_i - theta_z) + log(rizPlusOne/riz) * sin(theta_i - theta_z)
                
            end

            # Same for this array
            bNplus1 = -2*pi*freestream * cos(theta_i - alpha*pi/180)

            # Adding the Kutta conidtion to the bottom row of the 'A' matrix
            A[numPanels + 1,j] = A[numPanels + 1,j] + ANplus1j
            A[numPanels + 1,numPanels + 1] = A[numPanels + 1,numPanels + 1] + sum(ANplus1Nplus1)

            # Adding the Kutta condition to the last element of the 'b' array
            b[numPanels + 1] = b[numPanels + 1] + bNplus1
