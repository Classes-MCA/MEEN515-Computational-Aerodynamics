...
dphi = .05 # phi spacing for cosine spacing (keep small)

x = makeXarray(c,dphi) # make the cosine-spaced x-grid
e = 0 # max camber percentage
...
# Investigate: How to make the trig functions positive or negative appropriately?
# The angle of attack stuff only happens in the bottom row of A and all of b, so let's fix that

# Correction: It's when vorticity is introduced, through AOA or a nonsymmetric airfoil

# Regardless of AOA, the exact same A matrix is produced. This is good.
...
for i = 1:numPanels # i = row number
    ...
    for j = 1:numPanels # j = column number
        ...
        # Looks good. As it should , this varies only as a function of relative panel geometry to each other 
    end
    ...
    if i == 1 || i == numPanels
        for j = 1:numPanels
            ...
            # println("controlPanelSine = ", controlPanelSine)
            # println("controlPanelCosine = ", controlPanelCosine)
            A[numPanels + 1, numPanels + 1] = A[numPanels + 1, numPanels + 1] + ANplus1Nplus1_increment
            #A[numPanels + 1, numPanels + 1] = 0 # FIXME: Remove this
