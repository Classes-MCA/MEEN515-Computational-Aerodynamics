...
function miniXFOIL(e,p,t,dphi,angle,freestream,density,nu,showPlot)
    ...
    dPdz_bottom = -determinePressureGradient(reverse(pressureCoefficient[1:stagnationPointIndex],dims=1),bottomSurface,freestream,density) # Forced to be positive at the beginning by using the "-"
    # println(length(bottomSurface[1:end-1]))
    # println(length(dPdz_bottom))
    # scatter(bottomSurface[1:end-1],dPdz_bottom)
    dPdz_top    = -determinePressureGradient(pressureCoefficient[stagnationPointIndex:end],topSurface,freestream,density)
    ...
    Ve_top    = Velocity[stagnationPointIndex:end-1]
    ...
    # scatter(topSurface,Ve_top[1:end])
